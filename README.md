...
### 3. cat lab3.data | awk '/Susan/{print $1}'  | awk -F ':' '/Susan/{print $2}'

awk '/Susan/{print $1}' : печатает 1 столбец (имя) только в строке где есть Susan. Разделитель строк - пробел

awk -F '/Susan/{print $2}' : печатает 2 столбец (телефон) только в строке где есть Susan. Разделитель строк - :

### 4. cat lab3.data | awk '/^D/{print $2}'

^D : первой буквой должна быть D

awk '/^D/{print $2}' : печатает 2 столбец (фамилия) только в строке где фамилия начинается с буквы D. 

### 5. cat lab3.data | awk '/^C/ || /^E/ {print $1}' 

/^C/ || /^E/ : в начале буква C или буква E

### 6. cat lab3.data | awk -F ':' '/^(916)/ {print $1}'
